    ...
    <Layout>
      <div className="bg-white h-96 w-96 rounded-xl opacity-1 border border-gray-400 px-8 flex justify-between flex-col z-20">
        <div className="flex items-center justify-center gap-5 flex-col h-full">
          <h3 className="font-black text-2xl">Sign Up</h3>
          <h3 className="text-center text-sm text-slate-500">Sign up with your email and password to continue.</h3>
          <InputWithIcon placeholder="Email" leftIcon={<User className="text-gray-500" />} onChange = {(e)=> {setEmail(e.target.value)}}/>
          <InputWithIcon placeholder="Username" leftIcon={<User className="text-gray-500" />}  onChange = {(e)=> {setUsername(e.target.value)}}/>
          ...
          />
          <button type="button" onClick={handleSignUp} className="bg-black text-white font-black opacity-100 w-full border-gray-300 border py-3 rounded-md ">
          {
